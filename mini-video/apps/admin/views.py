...
from apps.admin.forms import LoginForm, TagForm, MovieForm, PreviewForm, PwdForm, AuthForm, RoleForm, AdminForm
from apps.models import Admin, Tag, Movie, Preview, User, Comment, Moviecol, Oplog, Adminlog, UserLog, Auth, Role
...
def admin_auth(func):
    ...
    def decorated_function(*args, **kwargs):
        ...
        if admin.is_super == 0:
            ...
            auths = list(map(lambda x: int(x), auths.split(",")))  # 登录者所拥有的权限
...
def movie_edit(id=None):
    ...
    if form.validate_on_submit():
        ...
        if movie_count and movie.title != data["title"]:
            flash("片名已经存在", "err")
...
def preview_edit(id=None):
    ...
    if form.validate_on_submit():
        ...
        if preview_count and preview.title != data["title"]:
            flash("预告名已经存在", "err")
...
def auth_edit(id=None):
    ...
    if form.validate_on_submit():
        ...
        if auth_count and auth.name != data["name"]:
            flash("权限名已经存在", "err")
...
def role_edit(id=None):
    ...
    if form.validate_on_submit():
        ...
        if role_count and role.name != data["name"]:
            flash("角色名已经存在", "err")
